...
var H='<div class="dtm-card">'
+'<div class="dtm-close">\u2715</div>'
...
+'<div class="dtm-step"><div class="dtm-check">\u2713</div><div class="dtm-h">\u00a1Mensaje enviado!</div><div class="dtm-sub">Te contactamos en menos de 24 horas h\u00e1biles.</div>'+nav(false,'Cerrar','dtm-closebtn')+'</div>'
+'</div>';
var ov=document.createElement('div');ov.className='dtm-overlay';ov.id='dtm';ov.innerHTML=H;document.body.appendChild(ov);
...
ov.querySelectorAll('.dtm-f').forEach(function(f){var t=f.getAttribute('data-t'),n=f.getAttribute('data-n');var el=document.createElement(t==='textarea'?'textarea':'input');if(t==='textarea'){el.rows=4;el.style.resize='vertical'}else{el.type=t}el.placeholder=n;el.style.cssText=IS;f.appendChild(el)});
var steps=ov.querySelectorAll('.dtm-step'),dots=ov.querySelectorAll('.dtm-dot'),cur=0;
...
function open(){ov.style.display='flex';document.body.style.overflow='hidden';show(0)}
function close(){ov.style.display='none';document.body.style.overflow=''}
document.querySelectorAll('a[href="#lt-contacto"]').forEach(function(a){a.addEventListener('click',function(e){e.preventDefault();open()})});
ov.querySelector('.dtm-close').addEventListener('click',close);
ov.querySelector('.dtm-closebtn').addEventListener('click',close);
ov.addEventListener('click',function(e){if(e.target===ov)close()});
document.addEventListener('keydown',function(e){if(e.key==='Escape'){if(ov.style.display==='flex')close()}});
ov.querySelectorAll('.dtm-chip').forEach(function(c){c.addEventListener('click',function(){c.classList.toggle('sel')})});
function vals(sel){var r=[];ov.querySelectorAll(sel+' .dtm-chip.sel').forEach(function(c){r.push(c.textContent.trim())});return r.join(', ')}
function inp(n){var f=ov.querySelector('.dtm-f[data-n="'+n+'"]');return f?f.querySelector('input,textarea'):null}
ov.querySelectorAll('.dtm-next').forEach(function(b){b.addEventListener('click',function(){
...
show(cur+1)})});
ov.querySelectorAll('.dtm-back').forEach(function(b){b.addEventListener('click',function(){show(cur-1)})});
ov.querySelector('.dtm-send').addEventListener('click',function(){
...
if(!form){fail();return}
var srv=vals('.dtm-servicios'),tls=vals('.dtm-tools'),msg=inp('Mensaje').value.trim();

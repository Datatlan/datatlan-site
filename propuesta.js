...
(function(){
  ...
  var state = { name:'', type:'', email:'', cname:'', phone:'', lada:'+52', sel:{} }; // sel[id] = 'crear' | 'mejorar'
  ...
  function submitLead(btn, cb){
    ...
    var msg = 'Giro: '+state.type+' | Frentes: '+frentesResumen()+(state.phone?(' | Tel: '+state.lada+' '+state.phone):'');
    ...
    hid('Giro',state.type); hid('Frentes',frentesResumen()); hid('Telefono', state.phone?(state.lada+' '+state.phone):'');
  ...
  $('#go-3').onclick=function(){
    ...
    state.cname=nm; state.email=em; state.phone=$('#phone').value.trim(); state.lada=ladaSel.value;
    var btn=this;
  ...
  $('#restart').onclick=function(){ state={name:'',type:'',email:'',cname:'',phone:'',lada:'+52',sel:{}};
    ['bizname','email','cname','phone'].forEach(function(id){$('#'+id).value='';});
    ladaSel.value='+52';
